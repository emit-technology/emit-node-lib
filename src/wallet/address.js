...
function getPublicKeyBs58(privateKey) {
    ...
    var pubKey = Buffer.from(pb.toRistrettoBytes());
    var prefix = Buffer.from(serial_1.ADDRESS_BYTES_PREFIX, "hex");
    var h0 = sign_1.blake2b(serial_1.EMIT_ADDR_BS_H0, Buffer.concat([prefix, pubKey]));
    var h0_r = Buffer.alloc(32, 0);
    h0_r.fill(h0, 0, h0.length > 32 ? 32 : h0.length);
    var h1 = sign_1.blake2b(serial_1.EMIT_ADDR_BS_H1, Buffer.concat([h0_r, pubKey]));
...
function getPublicKey(privateKey) {
    ...
    var pubKey = serial_1.ED_BASE.multiply(sk).toRistrettoBytes();
...
function fromAddressBytes(addr) {
    ...
    var h0 = sign_1.blake2b(serial_1.EMIT_ADDR_BS_H0, data.slice(0, 33));
    var h0_r = h0.slice(0, 32);
    var h1 = sign_1.blake2b(serial_1.EMIT_ADDR_BS_H1, Buffer.concat([h0_r, data.slice(1, 33)]));
